...
export class MockModalService extends ModalService {
    ...
    fakeModalInstance: IModalInstance<any> = {
        open: jasmine.createSpy('open').and.callFake(() => {
            return new Promise((resolve, reject) => {
                this.confirmLastModal = (value?: any) => {
                    resolve(value);
                    tick();
                };
                this.cancelLastModal = () => {
                    reject();
                    tick();
                };
            });
    ...
    confirmLastModal: (value?: any) => void = () => {
        throw new Error(
            'MockModalService.confirmLastModal() can only be called after a modal has been opened in a test!'
        );
    };
    ...
    cancelLastModal: () => void = () => {
        throw new Error(
            'MockModalService.cancelLastModal() can only be called after a modal has been opened in a test!'
        );
    };
