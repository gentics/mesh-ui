...
describe('MultiFileUploadDialogComponent', () => {
    ...
    it(
        'Should accept dropped files',
        fakeAsync(() => {
            const droppedFiles: File[] = initializeComponentWithFiles(fixture);
            const fileDropArea = fixture.debugElement.query(By.directive(FileDropArea));
            const newFiles = [new File([''], 'text1.txt', { type: 'text/plain' })];
    ...
    it(
        'Should be able to remove dropped files',
        fakeAsync(() => {
            const droppedFiles: File[] = initializeComponentWithFiles(fixture);
            const renderedFiles = fixture.debugElement.queryAll(By.css('.media-box'));
    ...
    it(
        'Should filter out the schemas with binary fields',
        fakeAsync(() => {
            ...
            const correctScehmas: Schema[] = [
                schemaWithBinary['1.0'],
                schemaWithTwoBinaries['1.0'],
                schemaWithTwoBinariesRequired['1.0']
            ];
            initializeComponentWithFiles(fixture);
            expect(
                fixture.componentInstance.availableSchemas.every(schema => correctScehmas.indexOf(schema) !== -1)
            ).toEqual(true);
        })
    );
});
