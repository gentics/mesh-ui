...
describe('AdminBreadcrumbsComponent', () => {
    ...
    it('ignores route with no breadcrumb defined', fakeAsync(() => {
        const sub = instance.breadcrumbs$.pipe(take(1)).subscribe();
        tick();
    ...
    it('creates links for top level breadcrumb', fakeAsync(() => {
        const sub = instance.breadcrumbs$.pipe(take(1)).subscribe(it => {
...
@Component({
    ...
    template: `
        <mesh-admin-breadcrumbs></mesh-admin-breadcrumbs> <router-outlet></router-outlet>
    `
})
class TestComponent {}
