...
@Component({
    selector: 'mesh-s3binary-field',
    templateUrl: './s3binary-field.component.html',
    styleUrls: ['./s3binary-field.scss']
})
export class S3binaryFieldComponent extends BaseFieldComponent {
    ...
    valueChange(value?: BinaryField): void {
        ...
        if (this.binaryProperties.file) {
            ...
            this.objectUrl = this.getS3BinaryUrl(this.binaryProperties);
    ...
    editImage(): void {
        ...
        if (newFile) {
            ...
            imageUrl = this.apiService.project.getS3BinaryFileUrl(
                node.project.name!,
                node.uuid,
                this.api.field.name,
                node.language!
